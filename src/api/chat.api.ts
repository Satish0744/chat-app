...
export const chatApi = {
  ...
  getMessages: async (chatId: string, page: number = 1, limit: number = 20): Promise<{ messages: Message[]; total: number; hasMore: boolean }> => {
  ...
  removeReaction: async (messageId: string, emoji: string): Promise<{ reactions: Record<string, string[]> }> => {
    const response = await axiosInstance.delete(`/chat/react/${messageId}`, {
      data: { emoji },
    });
    return response.data;
  },

  createGroup: async (name: string, members: string[], avatar?: File, description?: string): Promise<Chat> => {
    const formData = new FormData();
  ...
  leaveGroup: async (groupId: string): Promise<{ message: string }> => {
    const response = await axiosInstance.post(`/chat/group/leave/${groupId}`);
    return response.data;
  },

  deleteGroup: async (groupId: string): Promise<{ message: string }> => {
    const response = await axiosInstance.delete(`/chat/group/${groupId}`);
    return response.data;
  },
  ...
  searchMessages: async (chatId: string, query: string): Promise<Message[]> => {
    const response = await axiosInstance.get(`/chat/search/${chatId}`, {
      params: { query },
    });
    return response.data;
  },

  forwardMessage: async (messageId: string, chatIds: string[]): Promise<{ messages: Message[] }> => {
    const response = await axiosInstance.post(`/chat/forward/${messageId}`, { chatIds });
    return response.data;
  },

  pinMessage: async (messageId: string): Promise<{ message: string }> => {
    const response = await axiosInstance.post(`/chat/pin/${messageId}`);
    return response.data;
  },

  unpinMessage: async (messageId: string): Promise<{ message: string }> => {
    const response = await axiosInstance.delete(`/chat/pin/${messageId}`);
    return response.data;
  },
};